    ...
    <div class="max-w-md w-full relative z-10 space-y-8">
        <div class="text-center">
            ...
            <h2 class="mt-6 text-3xl font-display font-bold text-white">
                {{ TranslationHelper::get('create_account', 'Create your account') }}
            </h2>
            <p class="mt-2 text-sm text-white/80">
                {{ TranslationHelper::get('or', 'Or') }} <a href="{{ route('login') }}" class="font-medium text-accent hover:text-accent-dark transition-colors">{{ TranslationHelper::get('sign_in_existing', 'sign in to your existing account') }}</a>
            </p>
        </div>
        ...
        <div class="bg-white/10 backdrop-blur-md p-8 rounded-xl shadow-xl border border-white/10">
            ...
            @if(isset($registration_disabled) && $registration_disabled)
            <div class="bg-amber-500/20 border border-amber-500/50 text-white p-4 rounded-lg mb-6">
                <div class="font-medium text-center">{{ TranslationHelper::get('registration_paused', 'Registration Temporarily Paused') }}</div>
                <p class="mt-2 text-center">{{ TranslationHelper::get('registration_paused_message', 'New account registrations are temporarily disabled. Please try again later or contact support for assistance.') }}</p>
            </div>
            @else
            <form class="mt-8 space-y-6" action="{{ secure_url(route('register', [], false)) }}" method="POST">
                @csrf
                <div class="rounded-md space-y-4">
                    <div>
                        <label for="name" class="sr-only">{{ TranslationHelper::get('full_name', 'Full name') }}</label>
                        <input id="name" name="name" type="text" autocomplete="name" value="{{ old('name') }}" required class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-white/20 bg-white/10 text-white placeholder-white/60 focus:outline-none focus:ring-accent focus:border-accent focus:z-10 sm:text-sm" placeholder="{{ TranslationHelper::get('full_name', 'Full name') }}">
                    </div>
                    <div>
                ...
                <div>
                    <button type="submit" class="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-lg text-white bg-accent hover:bg-accent-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent transition-colors duration-200">
                        {{ TranslationHelper::get('create_account_button', 'Create account') }}
                    </button>
                </div>
                <div class="text-center text-xs text-white/60">
                    {{ TranslationHelper::get('terms_agreement', 'By creating an account, you agree to our') }} <a href="#" class="text-accent hover:text-accent-dark">{{ TranslationHelper::get('terms_of_service', 'Terms of Service') }}</a> {{ TranslationHelper::get('and', 'and') }} <a href="#" class="text-accent hover:text-accent-dark">{{ TranslationHelper::get('privacy_policy', 'Privacy Policy') }}</a>.
                </div>
            </form>
            ...
            <div class="mt-6">
                ...
                <div class="mt-6">
                    <a href="{{ route('auth.google') }}" class="group relative w-full flex justify-center py-3 px-4 border border-white/20 bg-white/5 hover:bg-white/10 text-sm font-medium rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent transition-colors duration-200">
                        ...
                        {{ TranslationHelper::get('sign_up_with_google', 'Sign up with Google') }}
                    </a>
                </div>
            </div>
            @endif
