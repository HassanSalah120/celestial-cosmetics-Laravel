...
<div class="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-b from-primary to-primary-light relative overflow-hidden">
    ...
    <div class="relative z-10 text-center max-w-lg bg-white/10 backdrop-blur-md p-8 sm:p-12 rounded-xl shadow-2xl border border-white/20">
        <h1 class="font-display text-7xl sm:text-9xl font-bold text-accent mb-4">{{ isset($exception) ? $exception->getStatusCode() : 'Error' }}</h1>
        <h2 class="font-display text-xl sm:text-2xl font-medium text-white mb-6">{{ isset($exception) ? $exception->getMessage() ?: 'Something Went Wrong' : 'Something Went Wrong' }}</h2>
        <p class="text-white/80 mb-8">A cosmic anomaly has occurred in our celestial system. We're working to restore the harmony of the universe.</p>
