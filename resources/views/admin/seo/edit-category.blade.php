...
@section('title', 'Edit Category SEO - ' . $category->name)

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-semibold text-gray-900 mt-4">Edit Category SEO</h1>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('admin.seo.categories') }}" class="ml-1 text-sm font-medium text-primary hover:text-primary-dark md:ml-2">Category SEO</a>
                </div>
            </li>
    ...
    <div class="bg-white shadow rounded-lg mb-6">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6 flex flex-wrap items-center justify-between">
            <div class="flex items-center">
                ...
                <h3 class="text-lg leading-6 font-medium text-gray-900">Edit SEO for: {{ $category->name }}</h3>
            </div>
            <div>
                <a href="{{ route('admin.seo.categories') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
        ...
        <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="mt-4 md:mt-0 md:ml-6">
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                                    <svg class="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                    Edit Category
                                </a>
                                <a href="{{ route('products.category', $category->slug) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                                    <svg class="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <form action="{{ route('admin.seo.update-category', $category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                   ...
                                   value="{{ old('meta_title', $category->meta_title) }}" maxlength="70">
                            <div id="titleCounter" class="mt-1 text-sm text-gray-500">
                                <span>Recommended: 50-60 characters. </span>
                                <span class="chars-count">{{ strlen(old('meta_title', $category->meta_title)) }}</span>/70
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="descriptionCounter" class="mt-1 text-sm text-gray-500">
                                ...
                                <span class="chars-count">{{ strlen(old('meta_description', $category->meta_description)) }}</span>/160
                            </div>
                            <div class="mt-1 text-sm text-gray-500">
                                Default: {{ Str::limit($category->seo_description, 100) }}
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            @elseif($category->image)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" 
                                         class="h-32 w-auto object-cover rounded-md border border-gray-200">
                                    <div class="mt-1 text-sm text-gray-500">Default: Using category image</div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md" 
                                ...
                                <option value="summary" {{ old('twitter_card_type', $category->twitter_card_type) === 'summary' ? 'selected' : '' }}>
                                    Summary Card
                                </option>
                                <option value="summary_large_image" {{ old('twitter_card_type', $category->twitter_card_type) === 'summary_large_image' ? 'selected' : '' }}>
                                ...
                                <div class="p-4">
                                    <div id="socialPreview" class="max-w-md mx-auto border border-gray-200 rounded-md overflow-hidden">
                                        <div class="h-52 bg-gray-100 flex items-center justify-center overflow-hidden">
                                            @if($category->og_image)
                                                <img src="{{ asset('storage/' . $category->og_image) }}" alt="OG Image" class="w-full h-full object-cover">
                                            @elseif($category->image)
                                        ...
                                        <div class="bg-white p-3">
                                            ...
                                            <div class="text-gray-500 text-xs mt-2 uppercase">
                                                {{ parse_url(route('products.category', $category->slug), PHP_URL_HOST) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="mt-6 flex items-center space-x-3">
                    ...
                    <a href="{{ route('admin.seo.categories') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                        Cancel
                    </a>
                </div>
            </form>
...
<script>
    $(document).ready(function() {
        ...
        $('#meta_title').on('input', function() {
            ...
            $('#previewTitle').text($(this).val() || '{{ $category->seo_title }}');
            $('#socialTitle').text($(this).val() || '{{ $category->seo_title }}');
        ...
        $('#meta_description').on('input', function() {
            ...
            $('#previewDescription').text($(this).val() || '{{ $category->seo_description }}');
            $('#socialDescription').text($(this).val() || '{{ $category->seo_description }}');
